...
from cuhkx_baseline.model import FusionNet
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=Path, default=ROOT / "outputs" / "fusion_best.pt")
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--output", type=Path, default=ROOT / "outputs" / "submission_fusion.csv")
    ...
    stats_npz = np.load(paths.outputs_dir / "fusion_stats.npz")
    stats = {key: stats_npz[key] for key in stats_npz.files}

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ckpt = torch.load(args.checkpoint, map_location=device, weights_only=False)
    params = ckpt.get("params", {})
    model = FusionNet(width=int(params.get("width", 128)), image_base=int(params.get("image_base", 32))).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    loader = DataLoader(TestFusionDataset(test_df, cache_dir, stats), batch_size=args.batch_size)
    paths_out = []
    preds = []
    with torch.no_grad():
        for skeleton, imu, depth, ir, paths_batch in tqdm(loader, desc="predict fusion"):
